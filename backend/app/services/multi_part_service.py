...
def part_ids(db: Session, multi_part_id: str) -> list[str]:
    return list(
        db.execute(
            select(MultiPartItem.part_id)
            .where(MultiPartItem.multi_part_id == multi_part_id)
            .order_by(MultiPartItem.part_id)
        )
        .scalars()
        .all()
    )
...
def _set_items(db: Session, multi_part_id: str, company_id: str, part_id_list: list[str]) -> None:
...
def create_multi_part(
    db: Session, payload: MultiPartCreate, company_id: str, actor_user_id: str | None
) -> MultiPart:
...
def list_multi_parts(db: Session) -> list[MultiPart]:
    return list(
        db.execute(
            select(MultiPart).where(MultiPart.is_active.is_(True)).order_by(MultiPart.custom_id)
        )
        .scalars()
        .all()
    )
...
def update_multi_part(
    db: Session, mp: MultiPart, payload: MultiPartUpdate, company_id: str, actor_user_id: str | None
) -> MultiPart:

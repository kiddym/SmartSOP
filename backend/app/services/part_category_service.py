...
def create_category(
    db: Session, payload: PartCategoryCreate, company_id: str, actor_user_id: str | None
) -> PartCategory:
    cat = PartCategory(name=payload.name, description=payload.description, company_id=company_id)
    db.add(cat)
    db.commit()
    db.refresh(cat)
    return cat
...
def list_categories(db: Session) -> list[PartCategory]:
    return list(
        db.execute(
            ...
            .order_by(PartCategory.name, PartCategory.id)
        )
        .scalars()
        .all()
    )

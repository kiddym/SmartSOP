...
def test_pm_create_defaults():
    p = PMCreate(title="月检", start_date="2026-06-01", frequency_unit="MONTH", frequency_value=1)
...
def test_pm_create_rejects_blank_title():
    with pytest.raises(ValidationError):
        PMCreate(title="", start_date="2026-06-01", frequency_unit="DAY", frequency_value=1)

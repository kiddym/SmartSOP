...
def test_store_from_token_writes_row_and_file(db: Session, storage_tmp: Path) -> None:
    ...
    row = source_docx_service.store_from_token(
        db, procedure_group_id="grp-1", upload_token=up.upload_token
    )
...
def test_store_from_token_degrades_without_token(db: Session, storage_tmp: Path) -> None:
    assert (
        source_docx_service.store_from_token(db, procedure_group_id="g", upload_token=None) is None
    )
    assert (
        source_docx_service.store_from_token(db, procedure_group_id="g", upload_token="ghost")
        is None
    )
...
def test_delete_for_group_removes_row_and_file(db: Session, storage_tmp: Path) -> None:
    ...
    source_docx_service.store_from_token(
        db, procedure_group_id="grp-9", upload_token=up.upload_token
    )

...
def test_update_draft_replaces_lines_and_scalars(db: Session):
    po = svc.create_purchase_order(
        db,
        _payload(notes="old", lines=[POLineCreate(part_id="p-1", quantity=Decimal("1"))]),
        CO,
        actor_user_id="a",
    )
    svc.update_purchase_order(
        ...
        PurchaseOrderUpdate(
            notes="new",
            lines=[POLineCreate(part_id="p-9", quantity=Decimal("2"), unit_cost=Decimal("4"))],
        ),
        CO,
        actor_user_id="a",
    )
...
def test_update_keeps_lines_when_omitted(db: Session):
    po = svc.create_purchase_order(
        db,
        _payload(lines=[POLineCreate(part_id="p-1", quantity=Decimal("1"))]),
        CO,
        actor_user_id="a",
    )

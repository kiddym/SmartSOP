...
def test_reading_below_threshold_no_wo(db: Session):
    ...
    _reading, wos = svc.submit_reading(
        db, m, MeterReadingCreate(value=Decimal("50")), CO, actor_user_id="a"
    )
    assert wos == []
    db.refresh(t)
...
def test_reading_crosses_threshold_fires_once(db: Session):
    m, t = _meter_with_trigger(db)
    _, wos1 = svc.submit_reading(
        db, m, MeterReadingCreate(value=Decimal("150")), CO, actor_user_id="a"
    )
    assert len(wos1) == 1
    db.refresh(t)
    assert t.is_armed is False  # 发火后解除武装
    # 持续超阈：不重复发火
    _, wos2 = svc.submit_reading(
        db, m, MeterReadingCreate(value=Decimal("160")), CO, actor_user_id="a"
    )
    assert wos2 == []

...
def _down(db, asset, started_at, ended_at=None):
    db.add(
        AssetDowntime(asset_id=asset.id, started_at=started_at, ended_at=ended_at, company_id=CO)
    )
    db.commit()
...
def test_filter_by_location(db: Session):
    ...
    r = svc.asset_reliability_dashboard(
        db, date_from=date(2026, 1, 1), date_to=date(2026, 1, 1), location_id="loc-1"
    )

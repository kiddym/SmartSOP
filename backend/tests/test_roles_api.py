...
def test_reject_unknown_permission(client):
    ...
    r = client.post(
        "/api/v1/roles",
        headers=_h(t),
        json={"code": "x", "name": "X", "permissions": ["does.not.exist"]},
    )
...
def test_update_role(client):
    ...
    r = client.patch(
        f"/api/v1/roles/{rid}", headers=_h(t), json={"permissions": ["user.view", "user.create"]}
    )
...
def test_cannot_delete_builtin(client):
    t = _admin(client)
    rid = next(
        x for x in client.get("/api/v1/roles", headers=_h(t)).json() if x["code"] == "admin"
    )["id"]

...
def _register(db, company="Acme", email="a@acme.com"):
    return auth_service.register(
        db, RegisterRequest(company_name=company, email=email, password="secret123", name="Alice")
    )
...
def test_login_ambiguous_email_requires_slug(db):
    auth_service.register(
        db, RegisterRequest(company_name="Acme", email="same@x.com", password="secret123", name="A")
    )
    auth_service.register(
        db,
        RegisterRequest(company_name="Globex", email="same@x.com", password="secret123", name="B"),
    )
    with pytest.raises(auth_service.AuthError):
        auth_service.authenticate(db, LoginRequest(email="same@x.com", password="secret123"))
    u = auth_service.authenticate(
        db, LoginRequest(email="same@x.com", password="secret123", company_slug="globex")
    )

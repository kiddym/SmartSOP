...
def test_transition_and_activities(client):
    ...
    r = client.post(
        f"/api/v1/work-orders/{wid}/transition", headers=_h(t), json={"to_status": "IN_PROGRESS"}
    )
    assert r.status_code == 200, r.text
    assert r.json()["status"] == "IN_PROGRESS"
    # 非法转移
    bad = client.post(
        f"/api/v1/work-orders/{wid}/transition", headers=_h(t), json={"to_status": "OPEN"}
    )
...
def test_update_and_delete(client):
    ...
    wid = client.post("/api/v1/work-orders", headers=_h(t), json={"title": "t"}).json()["id"]
    assert (
        client.patch(f"/api/v1/work-orders/{wid}", headers=_h(t), json={"priority": "HIGH"}).json()[
            "priority"
        ]
        == "HIGH"
    )
...
def test_sop_attach_execute_complete(client, db):
    ...
    # 挂接
    r = client.post(
        f"/api/v1/work-orders/{wid}/attach-procedure", headers=_h(t), json={"procedure_id": pid}
    )
    assert r.status_code == 200, r.text
    ...
    assert (
        client.patch(
            f"/api/v1/work-orders/{wid}/steps/{rid}", headers=_h(t), json={"is_done": True}
        ).status_code
        == 400
    )
    client.post(
    ...
    assert (
        client.post(
            f"/api/v1/work-orders/{wid}/transition", headers=_h(t), json={"to_status": "COMPLETE"}
        ).status_code
        == 400
    )
    assert (
...
def test_technician_execute_not_delete(client, db):
    admin = _admin(client)
    cid = _company_id(db, "acme")
    _seed_published_procedure(db, cid)
    client.post(
        "/api/v1/users",
        headers=_h(admin),
    ...
    # technician 不能建单
    assert (
        client.post("/api/v1/work-orders", headers=_h(tech), json={"title": "x"}).status_code == 403
    )
    # 能转状态（edit）
    assert (
        client.post(
            f"/api/v1/work-orders/{wid}/transition",
            headers=_h(tech),
            json={"to_status": "IN_PROGRESS"},
        ).status_code
        == 200

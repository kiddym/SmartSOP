...
def test_vendor_crud_and_parts(client):
    ...
    upd = client.patch(
        f"/api/v1/vendors/{vid}", json={"name": "改名", "part_ids": []}, headers=_h(t)
    )
...
def test_technician_can_view_not_create(client):
    ...
    assert client.post("/api/v1/vendors", json={"name": "x"}, headers=_h(tech)).status_code == 403


def test_vendor_tenant_isolation(client):
    a = _admin(client)
    vid = client.post("/api/v1/vendors", json={"name": "供应商A"}, headers=_h(a)).json()["id"]

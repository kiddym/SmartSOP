...
def test_assets_isolated(client):
    ...
    assert (
        client.patch(f"/api/v1/assets/{bid}", headers=_h(ta), json={"name": "hacked"}).status_code
        == 404
    )
...
def test_teams_and_categories_isolated(client):
    ...
    bcid = client.post("/api/v1/asset-categories", headers=_h(tb), json={"name": "B类"}).json()[
        "id"
    ]
    assert client.get("/api/v1/teams", headers=_h(ta)).json() == []
    assert client.get("/api/v1/asset-categories", headers=_h(ta)).json() == []
    assert (
        client.patch(f"/api/v1/teams/{btid}", headers=_h(ta), json={"name": "x"}).status_code == 404
    )
    assert (
        client.patch(
            f"/api/v1/asset-categories/{bcid}", headers=_h(ta), json={"name": "x"}
        ).status_code
        == 404
    )
...
def test_cross_tenant_downtime_404(client):
    ...
    assert (
        client.post(
            f"/api/v1/assets/{bid}/downtimes",
            headers=_h(ta),
            json={"started_at": "2026-05-30T08:00:00"},
        ).status_code
        == 404
    )

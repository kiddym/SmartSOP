...
def test_update_forbidden_for_unprivileged(client, db):
    ...
    client.post("/api/v1/auth/register", json={
        "company_name": "Acme", "email": "admin@acme.com", "password": "secret123", "name": "Admin"})
    ...
    tok = client.post("/api/v1/auth/accept-invite",
                      json={"token": raw, "name": "Member", "password": "memberpw1"}).json()["access_token"]
    h = {"Authorization": f"Bearer {tok}"}
    assert client.get("/api/v1/company-settings", headers=h).status_code == 200
    assert client.put("/api/v1/company-settings", headers=h, json={"timezone": "UTC"}).status_code == 403

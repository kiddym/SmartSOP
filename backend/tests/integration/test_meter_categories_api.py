...
def test_update_and_delete_category(client):
    t = _admin(client)
    cid = client.post("/api/v1/meter-categories", headers=_h(t), json={"name": "温度"}).json()["id"]
    r = client.patch(f"/api/v1/meter-categories/{cid}", headers=_h(t), json={"name": "压力"})
...
def test_cross_tenant_category_isolated(client):
    ...
    cid = client.post("/api/v1/meter-categories", headers=_h(tb), json={"name": "B类"}).json()["id"]
    ...
    assert client.delete(f"/api/v1/meter-categories/{cid}", headers=_h(ta)).status_code == 404
...
def test_technician_can_view_but_not_manage(client):
    ...
    assert (
        client.post("/api/v1/meter-categories", headers=_h(tech), json={"name": "x"}).status_code
        == 403
    )
...
def test_meter_can_set_and_read_back_category(client, db):
    ...
    # 改为空
    upd = client.patch(f"/api/v1/meters/{mid}", headers=_h(t), json={"meter_category_id": None})

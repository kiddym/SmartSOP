...
def upgrade() -> None:
    # 1) 删存量 NULL-company 行（child→parent，避免跨表 FK RESTRICT 干扰）。
    #    每表先计数并 WARNING 记录——该删除不可逆（downgrade 无法恢复），故不静默：
    #    若在某环境意外删到行，迁移输出会明确列出，便于在跑迁移前发现"假定无 prod 数据"被打破。
    bind = op.get_bind()
    # tb_folder.parent_id 是自引用 FK（ondelete=RESTRICT）。若 NULL-company 文件夹成树，
    # 单条 DELETE 在 MySQL 可能因父行先于子行被删而中止；先把待删行的 parent_id 置空，
    # 消除组内自引用，再整批删除（按层级正确性不再依赖删除顺序）。
    op.execute("UPDATE tb_folder SET parent_id = NULL WHERE company_id IS NULL")
    for table in _TENANT_TABLES:
        n = bind.execute(
            sa.text(f"SELECT COUNT(*) FROM {table} WHERE company_id IS NULL")
        ).scalar()
        if n:
            logger.warning(
                "sop_tenancy_hardening: 删除 %s 中 %s 行 company_id IS NULL（不可逆）",
                table,
                n,
            )

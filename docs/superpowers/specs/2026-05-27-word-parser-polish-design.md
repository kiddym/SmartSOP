...
**P3 改动后强制走一次 §5 MCP 抽样验收**（动 `import_blocks` 形态，前端可能炸），不通过则 revert。
...
| R2 | 融合式 `N.N、xxx：正文` 子标题要切块才能升 R → 改动到 `import_blocks` 形态 | 列为 P3，每次改动**强制** §5 MCP 抽样；切块改动单独 commit |
...
| R4 | 前端 `ProcedureEditorView` 对 `import_blocks` 字段强假设（`types/parse.ts`），后端字段调整会炸 | 任何 schema 改动**先**走 §5 MCP 抽样；后端字段不轻易改 |
